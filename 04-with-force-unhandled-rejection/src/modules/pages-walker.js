...
const { ResourcesPool } = require('./resources-pool');
...
class PagesWalker {
  ...
  async run(
    ...
    const pagesPool = new ResourcesPool(pages);

    const urlsQueue = new Queue(urls);
    const pagesPoolSize = this._configProvider.getPagesPoolSize();
    await new PromiseQueue(
      urlsQueue,
      pagesPoolSize,
    ).run(async url => {
      const page = pagesPool.take();
      await this._loadPage(url, page, handlePage);
      pagesPool.free(page);
    })
